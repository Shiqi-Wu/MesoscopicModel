...
def augment_npz_with_metrics(
    ...
    print(
        f"[load] loaded {npz_path}, traj shape: {traj.shape}, times shape: {times.shape}"
    )
    ...
    meta["metrics"] = {
        ...
        "free_energy": {
            "shape": list(F.shape),
            "desc": "coarse-grained free energy F[m(t)]",
        },
    }
...
def main(
    ...
    for r in range(n_rounds):
        ...
        if not npz_path.exists() or not json_path.exists():
            ...
            continue
        augment_npz_with_metrics(
            npz_path, json_path, beta, h_field, J0, kernel_k, remote_round_dir
        )
