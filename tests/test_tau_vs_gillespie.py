...
def test_compare_methods(model, spin_init, beta=0.4, J0=1.0, h=0.0,
                         epsilon=0.05, t_end=5.0, snapshot_dt=0.1,
                         n_runs=5, stem="test_results", output_dir="."):
    """
    ...
    for run_idx in range(n_runs):
        ...
        times_t, Ms_t, Es_t, snaps_t = model.simulate_kac_tauleap(
            spin_init.copy(), beta, J0, h, epsilon,
            t_end=t_end, snapshot_dt=snapshot_dt, return_snapshots=True, verbose_every=10
        )
        ...
        times_g, Ms_g, Es_g, snaps_g = model.simulate_kac(
            spin_init.copy(), beta, J0, h, epsilon,
            t_end=t_end, snapshot_dt=snapshot_dt, return_snapshots=True, verbose_every=1000
        )
    ...
    axs[0,0].errorbar(times, Ms_g_mean, yerr=Ms_g_std, label=f"Gillespie (N={n_runs})", 
                      alpha=0.8, errorevery=len(times)//10)
    axs[0,0].errorbar(times, Ms_t_mean, yerr=Ms_t_std, label=f"Tau-leaping (N={n_runs})", 
                      linestyle='--', alpha=0.8, errorevery=len(times)//10)
    ...
    axs[0,1].errorbar(times, Es_g_mean, yerr=Es_g_std, label=f"Gillespie (N={n_runs})", 
                      alpha=0.8, errorevery=len(times)//10)
    axs[0,1].errorbar(times, Es_t_mean, yerr=Es_t_std, label=f"Tau-leaping (N={n_runs})", 
                      linestyle='--', alpha=0.8, errorevery=len(times)//10)
    ...
    print(f"Average Gillespie time: {avg_gillespie_time:.2f}s (total: {total_gillespie_time:.2f}s)")
    print(f"Average Tau-leaping time: {avg_tauleap_time:.2f}s (total: {total_tauleap_time:.2f}s)")
...
if __name__ == "__main__":
    ...
    print(f"Starting comparison with {n_runs} independent runs using the same initial condition")
    test_compare_methods(model, spin_init, beta=beta, epsilon=epsilon, h=h,
                         t_end=t_end, snapshot_dt=snapshot_dt, n_runs=n_runs,
                         stem=stem, output_dir=output_dir)
