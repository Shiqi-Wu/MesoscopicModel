...
from utils import (
    ensure_dir,
    critical_temp_2d,
    export_npz_and_json_bundle,
)


def main():
    parser = argparse.ArgumentParser(
        description="Continuous-time Glauber simulation (NN and Kac)"
    )
    ...
    parser.add_argument(
        "--method", type=str, default="gillespie"
    ...
    ensure_dir(out_root)
    stem_base = (
        f"ct_glauber_L{L}_ell{args.ell:g}_sigma{args.sigma:g}_tau{args.tau:g}_m0{args.m0:g}"
    ...
    print(
        f"[info] L={L}, T={T:.4f} (Tc={Tc:.4f}, T/Tc={T/Tc:.3f}), "
    ...
    plot_initial_and_coarse(
        spins0, coarse0, timestr="spectral init", save_path=str(fig_path)
    )
    ...
    for r in range(args.rounds):
        ...
        meta = dict(
            ...
            snapshot_dt=float(args.snapshot_dt),
            init="spectral",
            dynamics="continuous_glauber",
            kernel=args.kernel,
