...
def grf_gaussian_spectrum_m_field(L: int,
                                  ell: float,
                                  sigma: float = 1.0,
                                  m0: float = 0.0,
                                  tau: float = 1.0,
                                  k_cut=None, butter_n=4, hard_cut=False,
...
def sample_spins_from_m_field(m_field: np.ndarray, seed: Optional[int] = None) -> np.ndarray:
...
# ---------- Plot helpers ----------
def plot_initial_and_coarse(spins_2d: np.ndarray,
...
if __name__ == "__main__":
    ...
    m_field = grf_gaussian_spectrum_m_field(L=L, ell=ell, sigma=sigma, m0=m0, tau=tau, seed=seed)
    ...
    # 4) plot both
    plot_initial_and_coarse(spins0, coarse0, timestr="spectral init", save_path="results/init_grf_ifft_test/initial_and_coarse.png")
